const Livro = require('../src/modules/gestao_livros/models/livro.model');
...
describe('Livro API', () => {
    ...
    describe('Cadastro', () => {
        test('✅ Deve criar um livro com sucesso', async () => {
            ...
            expect(response.body.novoLivro).toHaveProperty('id');
            expect(response.body.novoLivro.titulo).toBe('O Senhor dos Anéis');
            expect(response.body).toHaveProperty('msg', 'Livro criado com sucesso');
            livroId = response.body.novoLivro.id;
        });
    });
